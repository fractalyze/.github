...
The threshold is a *fraction of budget* rather than a fixed slack because a
shared remote executor's run-to-run spread for one unchanged target is wide
enough that any single measurement may be the fast one. The rule, the measured
spread behind it, and why cached results still count are the playbook's
(`sections/measurement.md`).
...
It runs against any repo's checkout rather than the one it ships in, so the
workspace is an argument:

    bazel test //... --build_event_json_file=/tmp/bep.json
    python3 check_test_budgets.py --bep /tmp/bep.json --repo-root .
"""
...
def query_budgets_xml(bazelrc: str | None, repo_root: Path) -> str:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=".",
        type=Path,
        help="the consuming repo's checkout — this script ships outside it",
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    # Refuse loudly rather than mis-measure — but a refusal is still a verdict,
    # and `warn` means this check never decides a build. A manual or scheduled
    # run that happens to carry the flag would otherwise hard-fail on a
    # measurement nobody asked it to police.
    ...
    xml_text = (
        ...
        if args.query_xml
        else query_budgets_xml(args.bazelrc, repo_root)
    )
    budgets = parse_budgets(xml_text, repo_root)
