...
import check_test_budgets as budgets
...
class MainTest(absltest.TestCase):
    ...
    def test_a_test_timeout_override_still_respects_warn(self) -> None:
        """A refusal is a verdict, and `warn` means this check decides nothing.

        The refusal above is right, but it used to be unconditional. A manual
        or scheduled run that happens to carry the flag would then hard-fail on
        a measurement nobody asked it to police — the same shape as a mode
        selection that denylists the events allowed to pass.
        """
        bep = self.create_tempfile(
            content=_bep(
                _options("--test_timeout=1200"),
                _result("//pkg:a", 20_000),
            )
        )
        self.assertEqual(
            budgets.main(
                ["--bep", bep.full_path, "--query-xml", self._xml(), "--mode", "warn"]
            ),
            0,
        )

    def _annotation_for(self, repo_root: str) -> str:
        bep = self.create_tempfile(content=_bep(_result("//pkg:a", 55_000)))
        out = io.StringIO()
    ...
    def test_the_workspace_is_an_argument_not_this_file_s_location(self) -> None:
        """This script ships outside the repo it measures.

        It used to derive the workspace from `__file__`, which is right only
        while the script lives in the repo under test. A shared action does
        not, and the failure is quiet: a workspace that does not match strips
        no prefix, so the annotation points at an absolute path GitHub cannot
        attach to a file.
        """
        self.assertIn("file=pkg/BUILD.bazel", self._annotation_for(str(WORKSPACE)))

    def test_a_workspace_that_does_not_match_leaves_the_path_absolute(self) -> None:
        """The negative half — without it the assertion above proves nothing.

        `parse_budgets` strips a prefix rather than resolving a path, so a
        wrong root is not an error anywhere; it just silently stops matching.
        """
        self.assertIn("file=/w/pkg/BUILD.bazel", self._annotation_for("/elsewhere"))
